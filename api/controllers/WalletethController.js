    ...
    if (email !== undefined && password !== undefined) {
      ...
      Users.findOne({email:email}).exec(function (err,result){
        if (result!==undefined){
          ...
          try {
            Wallets.findOne({email:email,status:'active',coin:'eth'}).exec(function (err,result){
              if (result==undefined){
                const data = {
                  "passphrase":user,
                  "label":user
                };
                bitgo.coin('teth').wallets().generateWallet(data,function (err, response) {
                  if (!err) {
                    ...
                    Wallets.create(
                      ...
                      if (!err){
                        QRCode.toDataURL(address, {width: 300}, function (err, url) {
                          return res.view('dashboard/walleteth', {title: title, params: params, url: url,address:address})
                        })
                        bitgo.coin('teth').wallets().get({ id:walletid })
