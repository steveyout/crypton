    ...
    if (email !== undefined && password !== undefined) {
      ...
      Users.findOne({email:email}).exec(function (err,result){
        if (result!==undefined){
          ...
          try {
            Wallets.findOne({email:email,status:'active',coin:'ltc'}).exec(function (err,result){
              if (result==undefined){
                const data = {
                  "passphrase":user,
                  "label":user
                };
                bitgo.coin('tltc').wallets().generateWallet(data,function (err, response) {
                  if (!err) {
                    ...
                    Wallets.create(
                      ...
                      if (!err){
                        QRCode.toDataURL(address, {width: 300}, function (err, url) {
                          return res.view('dashboard/walletltc', {title: title, params: params, url: url,address:address})
                        })
                        bitgo.coin('tltc').wallets().get({ id:walletid })
